...
st.set_page_config(page_title="Розподілене колективне ранжування на основі виконання лабораторних робіт №1-4", layout="wide")
...
HELP_TEXT = {
    ...
    "Голосування за евристики": (
        ...
        "Оберіть 3 різні евристики у порядку пріоритету (1-а найважливіша).\n"
    ...
    "Генетичний алгоритм": (
        ...
        "К1 мінімізує суму відстаней до всіх ранжувань.\n"
        "К2 мінімізує максимальну відстань.\n"
        "Натисніть «Запустити ГА» для запуску."
    ),
    "Визначення колективного ранжування об’єктів": (
        "Повний перебір перестановок.\n\n"
        "Знаходить медіани за критерієм мінімуму суми або максимуму відстаней.\n"
        "Підтримується зупинка і відновлення обчислень.\n"
    ...
    "Зміна ранжувань": (
        "Зміна індивідуальних ранжувань\n\n"
        "Оберіть голос конкретного експерта і змініть порядок обраних об'єктів.\n"
        "Система автоматично перераховує медіану і показує як змінився результат."
    ),
    "Адмін": (
...
if tab == "Голосування":
    with st.expander("Допомога"):
        st.markdown(HELP_TEXT["Голосування"])

    st.title("Експертне голосування")
    ...
    st.write("Введіть ім'я і оберіть 3 об'єкти у порядку пріоритету")
...
elif tab == "Результати голосування":
    with st.expander("Допомога"):
        st.markdown(HELP_TEXT["Результати голосування"])

    st.title("Результати голосування")
    st.markdown(f"**Предметна область:** {current_domain['name']}")
    ...
    output = io.StringIO()
    output.write(f"Результати голосування - {current_domain['name']}\n\n")
    ...
    if st.button("Запустити ГА"):
        log_action("Користувач", "Запущено ГА")
...
elif tab == "Визначення колективного ранжування об’єктів":
    with st.expander("Допомога"):
        st.markdown(HELP_TEXT["Визначення колективного ранжування об’єктів"])

    st.title("Визначення колективного ранжування")
    data_mode = st.radio(
        "Оберіть набір даних:",
    ...
    if col_run.button("Запустити перебір", key="run_brute"):
        # Обнуляємо всі дані перед новим запуском
        st.session_state["lr3_state"] = {
            "status": "running", "idx": 0,
            "min_sum": float("inf"), "min_max": float("inf"),
            "best_sum": [], "best_max": [], "samples": [],
            "dyn_sum": []
        }
        st.rerun()

    if col_pause.button("Пауза / Відновити", key="pause_btn"):
        if st.session_state["lr3_state"]["status"] == "running":
            st.session_state["lr3_state"]["status"] = "paused"
        elif st.session_state["lr3_state"]["status"] == "paused":
            st.session_state["lr3_state"]["status"] = "running"
        st.rerun()

    if col_reset.button("Скинути", key="reset_btn"):
        st.session_state["lr3_state"]["status"] = "idle"
        st.rerun()

    state = st.session_state["lr3_state"]

    if state["status"] in ["running", "paused", "finished"]:
        progress_val = state["idx"] / n_fact if n_fact > 0 else 0
        st.progress(progress_val)
        st.write(f"Оброблено комбінацій: {state['idx']} з {n_fact}")

        if state["status"] == "paused":
            st.warning("Обчислення призупинено")

        elif state["status"] == "running":
            dist_fn = cook_distance_e1 if heuristic_key == "E1" else cook_distance_e2
            CHUNK_SIZE = 5000
            perms_gen = itertools.permutations(winners)
            chunk = itertools.islice(perms_gen, state["idx"], state["idx"] + CHUNK_SIZE)
            ...
            for perm in chunk:
                ...
                if state["idx"] < 50:
                    row = {"№": state["idx"] + 1, "Перестановка": " > ".join(p_list)}
                    for i, d in enumerate(dists): row[f"d{i + 1}"] = d
                    row["Сума"] = s
                    row["Макс"] = m
        ...
        if state["status"] == "finished":
            ...
            st.dataframe(pd.DataFrame(state["samples"]), use_container_width=True, hide_index=True)
            ...
            output.write(f"\nМін. макс.: {state['min_max']}\nМедіани (макс.):\n")
            for p in state["best_max"]: output.write("  " + " > ".join(p) + "\n")
...
elif tab == "Розподілені обчислення компромісних ранжувань та визначення індексів задоволеності експертів колективним розв’язком":
    with st.expander("Допомога"):
        st.markdown(HELP_TEXT["Розподілені обчислення компромісних ранжувань та визначення індексів задоволеності експертів колективним розв’язком"])

    st.title("Розподілені обчислення компромісних ранжувань та визначення індексів задоволеності експертів колективним розв’язком")
    df_h = load_h_votes()
    if len(df_h) == 0:
        ordered_keys = list(HEURISTICS.keys())
    else:
    ...
    with col2:
        ...
        if st.button("Обчислити індекси", key="lr4_calc_a"):
            ...
            if len(df_sat) > 0:
                ...
                ax.set_ylabel("Задоволеність %", color="white");
                ax.set_xlabel("Експерт", color="white")
    ...
    st.title("Зміна індивідуальних ранжувань")
    st.markdown("Оберіть голос і змініть порядок об'єктів, система покаже, як змінюється медіана")
    ...
    if st.button("Перерахувати медіану"):
        ...
        if orig_best_s[0] == new_best_s[0]:
            st.success("Медіана не змінилась")
        else:
    ...
    if password == ADMIN_PASSWORD:
        ...
        st.divider()
        st.subheader("Журнал дій (протокол адміністратора і користувачів)")
        ...
        st.divider()
        st.subheader("Збереження / Завантаження архіву даних")
        ...
        log_action("Невідомий", "Невдала спроба входу")
